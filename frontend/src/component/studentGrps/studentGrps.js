...
export default class studentGrps extends Component {
  ...
  retrieveStudentGrps(){
    axios.get("http://localhost:8000/studentGrps").then(res => {
      console.log("hello3");
      if (res.data.success) {
        this.setState({
  ...
  onDelete = (id) =>{
    ...
    .then((willDelete) => {
      ...
      if (willDelete) {
    axios.delete(`http://localhost:8000/studentGrps/delete/${id}`).then((res)=>{
      swal("Delete Successfully!", "Student details are removed", "success");
    ...
    filterData(studentGrps,searchKey){
      const result =studentGrps.filter((studentGrps)=>
  ...
  render() {
    return (
      <div className='container'>
        <h1 className="text-center" style={{borderStyle: "solid", backgroundColor: "MidnightBlue", color: "orange"}}>Student Groups</h1>
        ...
        <table className="table table-striped">
          ...
          <tbody>
            {this.state.studentGrps.map((studentGrps, index) =>(
                <tr key={index}>
                  <th scope='row'>
                  <a href={`/studentGrp/${studentGrps._id}`}  style={{textDecoration:'none'}}>
                      Grp_{index + 1}
                   </a>   
                  </th>
                  ...
                  <a className="btn btn-warning" href={`/update/${studentGrps._id}`}>
                    <i className='fas fa-edit'> </i>&nbsp; Edit
                  </a>
                  &nbsp;
                  <a className="btn btn-danger" href="#" style={{  color: 'black'}} onClick={()=>this.onDelete(studentGrps._id)}>
                    <i className='far fa-trash-alt' style={{ textDecoration: 'none', color: 'black', fontSize:'medium' }}> </i> &nbsp; Delete
                  </a>
       ...
       <button className="btn btn-primary btn-lg active" style={{backgroundColor: "#c99212"}}>
           <a href='/add' style={{ textDecoration: 'none', color: 'white', fontSize:'large'}}>Create Student Group</a> 
       </button>

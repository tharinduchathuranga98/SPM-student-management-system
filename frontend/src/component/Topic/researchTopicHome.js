...
retrievePosts(){
  axios.get("/api/postsTopic").then(res =>{
    if(res.data.success){
      this.setState({
...
.then((willDelete) => {
  ...
  if (willDelete) {
axios.delete(`/api/postTopic/delete/${id}`).then((res)=>{
  swal("Deleted Successful", "Research Topic Details Are Removed", "success");
  ...
  render() {
    return (
      <div ><br></br>
        ...
        <h1 className="adminletter" style = {{color:'navy'}}><b>All Research Topic Details </b></h1>
        ...
        {this.state.posts.map((posts,index)=>(
              <tr key={index}>
                ...
                <td>
                  <a href={`/api/postTopic/${posts._id}`} style={{textDecoration:'none'}}>
                  {posts.studentsGrpId}
                  </a>
                  </td>
